...
class FollowUpProposal(NIABaseModel):
    ...
    title: str = Field(default="Commitment Follow-up", description="Concise title for the proposed action")
    description: str = Field(default="", description="Detailed explanation of the proposed follow-up")
    message: str = Field(default="", description="Human-readable notification or follow-up prompt")
    reason: str = Field(default="Standard commitment lifecycle follow-up", description="Underlying justification or context")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Timestamp proposal was generated")
    requires_approval: bool = Field(default=True, description="Strict Safe Action Gate requirement: always True")
